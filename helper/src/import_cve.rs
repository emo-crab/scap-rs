...
pub fn import_from_archive(
    ...
    match Cve::create(connection, &new_post) {
        Ok(cve_id) => {
            // 插入cpe_match关系表
            for vendor_product in cve_item.configurations.nodes.iter().flat_map(|n| n.vendor_product()).collect::<HashSet<_>>() {
...
pub fn import_from_api(
    ...
    match Cve::create_or_update(connection, &new_post) {
        Ok(cve_id) => {
            // 插入cpe_match关系表
            for vendor_product in configurations.iter().flat_map(|n| n.vendor_product()).collect::<HashSet<_>>() {
                import_vendor_product_to_db(connection, vendor_product.clone());
                create_cve_product(
                    connection,
                    cve_id.id.clone(),
